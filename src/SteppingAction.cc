...
namespace
{
G4double PmtQuantumEfficiency(G4double photonEnergy)
{
  ...
  auto upper = std::upper_bound(energy.begin(), energy.end(), photonEnergy);
  std::size_t i = std::distance(energy.begin(), upper) - 1;
...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  const G4double quantumEfficiency =
      PmtQuantumEfficiency(track->GetKineticEnergy());
